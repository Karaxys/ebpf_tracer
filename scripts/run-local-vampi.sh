...
LOCAL_AGENT_ENV="${LOG_DIR}/local-agent.env"

if [[ -z "${KARAXYS_AGENT_TOKEN:-}" && -z "${KARAXYS_ENROLLMENT_TOKEN:-}" && -f "${LOCAL_AGENT_ENV}" ]]; then
  # Local developer convenience only. File is written 0600 after registration.
  # shellcheck disable=SC1090
  source "${LOCAL_AGENT_ENV}"
fi
...
if [[ -n "${KARAXYS_BACKEND_URL:-}" && -z "${KARAXYS_AGENT_TOKEN:-}" && -n "${KARAXYS_ENROLLMENT_TOKEN:-}" ]]; then
  ...
  umask 077
  {
    printf 'KARAXYS_AGENT_TOKEN=%q\n' "${KARAXYS_AGENT_TOKEN}"
    printf 'KARAXYS_AGENT_ID=%q\n' "${REGISTERED_AGENT_ID}"
  } >"${LOCAL_AGENT_ENV}"
...
if [[ -n "${KARAXYS_BACKEND_URL:-}" && -n "${KARAXYS_AGENT_TOKEN:-}" ]]; then
  AGENT_ARGS+=(
    ...
    -remote-config-interval 30s
  )
fi

sudo -E "${BIN_DIR}/agent" "${AGENT_ARGS[@]}"
